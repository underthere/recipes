...
class client_pool : public std::enable_shared_from_this<
  ...
  template <typename T>
  using return_type =
      Expected<typename lazy_hacker<decltype(std::declval<T>()(
 ...
 public:
  ...
  async_simple::coro::Lazy<return_type<T>> send_request(
    ...
    if (!client) {
      co_return return_type<T>{UnExpect, std::errc::connection_refused};
    }
 ...
 private:
  ...
  async_simple::coro::Lazy<return_type_with_host<T>> send_request(
    ...
    auto client = co_await get_client(client_config);
    if (!client) {
      co_return return_type_with_host<T>{UnExpect,
